...
public class ConversationalSessionAttributeStore implements SessionAttributeStore{//, InitializingBean {
    ...
    private int keepAliveConversations = 50;//TODO ver un numero razonable!
	...
	if (keepAliveConversations > 0 && sessionConversationsMap.size() >= keepAliveConversations
	    ...
	    logger.warn("Se elimina la conversacion mas vieja, se superaron las 10 conversaciones simultaneas en la session. Verificar que se este llamando a status.setComplete() en los controller que utilizan @SessionAttributes");
	}
