			  ...
			  <div class="panel-body my-panel-body">
				<div class="form-group form-group-sm">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<html:inputDisabled name="codigo" base="${modulo}.${clase}.${labelAccion}"/>
			  		</div>
				</div>
				<div class="form-group form-group-sm">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<html:inputDisabled name="descripcion" base="${modulo}.${clase}.${labelAccion}"/>
			  		</div>
				</div>
				<div class="form-group form-group-sm">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">	
				...
				<ol id='checkboxes'>
					<li class='expanded'> All
			          <ol>
				      	<c:forEach items="${privilegiosDTOs}" var="modulo">
					      	<li>
				              <spring:message code="modulo.${modulo.value.name}"/>
				              <ol>
					              <c:forEach items="${modulo.value.submenu}" var="clase">
						            <li>
						            	<spring:message code="menu.${modulo.value.name}.${clase.value.name}" htmlEscape="false"/><span class="label label-info"><spring:message code="menu.${modulo.value.name}.${clase.value.name}.help" text=""/></span>
						            	<ol>
							            	<c:forEach items="${clase.value.submenu}" var="action">
							            		<li><input type="checkbox" disabled="disabled" id="privilegios${action.value.privilegio.ordinal}.checked" name="privilegios[${action.value.privilegio.ordinal}].checked" ${entidadDTO.privilegios[action.value.privilegio.ordinal].checked?'checked':''}/><spring:message code="menu.${modulo.value.name}.${clase.value.name}.${action.value.privilegio.accion}" arguments="," htmlEscape="false"/>
							            		<span class="label label-info"><spring:message code="menu.${modulo.value.name}.${clase.value.name}.${action.value.privilegio.accion}.help" text=""/></span></li>
